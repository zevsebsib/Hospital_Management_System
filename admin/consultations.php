...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    verifyCsrf('/hms/admin/consultations.php');
    $a = $_POST['action'] ?? '';
    if ($a === 'add') {
    ...
    header('Location: /hms/admin/consultations.php'); exit;
}
    ...
    <table>
      ...
      <?php if ($consults): foreach ($consults as $c): ?>
        <tr>
          ...
          <td>
            <a href="/hms/admin/prescriptions.php?consult_id=<?= $c['id'] ?>" class="btn btn-primary btn-sm">+ Rx</a>
          </td>
        </tr>
      <?php endforeach; else: ?>
